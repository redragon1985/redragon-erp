		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					// 创建不带有拖动的html
					//dongbin 修改2016.6.28 去掉form标签
					//html += '<form id="uploadForm" action="'+para.url+'" method="post" enctype="multipart/form-data">';
					html += '	<div id="uploadForm" class="upload_box">';
					html += '		<div class="upload_main single_main">';
		            //html += '			<div class="status_bar">';
		            //html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            //html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            //dongbin 修改 2015.12.30 去掉默认的选择文件和上传按钮
				    ...
				    html += '							<img class="upload_image" src="module/zyUpload/control/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					//dongbin 2016.1.8  修改 增加上传进度层
				    html += '<div class="uploadStatusClass" style="display:none;"><div class="jdt">';
                    html += '上传中';
                    html += '</div>';
					html += '<div class="img-up-mask"></div></div>';
			 ...
			 * 返回: 预览的html
			 */
			this.funDisposeDefaultPreviewHtml = function(src,index){
				var html = "";
				var imgWidth = parseInt(para.itemWidth.replace("px", ""))-15;
				...
				if (src != null && src != "") {
					...
					html += '	<a style="height:'+para.itemHeight+';width:'+para.itemWidth+';" href="javascript:void(0)" class="imgBox">';
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
					html += '			<img id="uploadImage_'+index+'" class="upload_image" src="'+src+'" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';                                                                 
					html += '		</div>';
					html += '	</a>';
			...
			this.createCorePlug = function(){
				var imageSrc;
				//初始化默认已上传图片展示，一下3方法用于此功能
				if(typeof(para.imageSrc) == 'string'){
					imageSrc = [para.imageSrc];
				}else{
					imageSrc = para.imageSrc;
				}
				for(var i in imageSrc){
					var previewHtml = self.funDisposeDefaultPreviewHtml(imageSrc[i],10000+i);
					funAppendDefaultPreviewHtml(previewHtml,10000+i);
				}
				
				// 添加预览html
				function funAppendDefaultPreviewHtml(html,index){
					// 添加到添加按钮前
					if(para.dragDrop){
						$("#preview").append(html);
					}else{
						$(".add_upload").before(html);
						if($(".upload_append_list:visible").size()>=1){
							$(".add_upload").hide();
						}
					}
					// 绑定删除按钮
					funBindDelEvent(index);
					funBindHoverEvent();
				};
				...
				function funBindDelEvent(index){
					if($(".file_del").length>0){
						...
						$(".file_del").click(function() {
							...
							//删除对应的input file
							$("#uploadList_"+parseInt(index)).remove();
							return false;	
						});
					}
				};
				...
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funDealtPreviewHtml = function() {
							file = selectFiles[i];
							if (file) {
								//dongbin 2016.1.10 修改 file id编号
						...
						var funAppendPreviewHtml = function(html){
							...
							if(para.dragDrop){
								...
								//dongbin 修改 2015.12.30 控制只能上传3张图片
								if($(".upload_append_list:visible").size()>=2){
									$(".add_upload").hide();
								}else{
									//dongbin 修改 2015.1.8 修改 添加多个file组件
									//克隆file组件
									var old_input_file = $("#fileImage");
									input_file = $("#fileImage").clone();
									
									//修改旧组件id
									$(old_input_file).attr("id","inputFile_"+file_index);
									//隐藏旧组件
									$(old_input_file).hide();
									//将新组件添加到页面
									$(input_file).insertBefore($("#uploadForm"));
									//给新组建添加事件
									ZYFILE.init($("#fileImage"));
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									...
									//删除对应的input file
									$("#inputFile_"+parseInt($(this).attr("data-index"))).remove();
