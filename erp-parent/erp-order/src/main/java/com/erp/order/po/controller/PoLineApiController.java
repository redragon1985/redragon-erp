...
@RestController
@RequestMapping("/api/poLine")
public class PoLineApiController extends ControllerSupport{
    
    //日志处理
    private Logger logger = LoggerFactory.getLogger(PoLineApiController.class);
    ...
    public String getPoLineList(Pages pages, PoLineCO poLineCO) {
        //TODO: 分页查询数据
        if(pages.getPage()==0) {
            pages.setPage(1);
        }
        ...
        //分页查询数据
        List<PoLine> list = this.poLineService.getDataObjects(pages, poLineCO);
    ...
    public String getPoLine(PoLine poLine) {
        //TODO: 查询要编辑的数据
    ...
    public String editPoLine(@Valid PoLine poLine, BindingResult bindingResult) {
        //参数校验
        ...
        //保存编辑的数据
        this.poLineService.insertDataObject(poLine);
