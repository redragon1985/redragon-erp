						...
						<ul class="dropdown-menu dropdown-user">
                        	<c:choose>
                        		<c:when test="${requestScope.payHead.approveStatus=='APPROVE' }">
                        			<li><a href="javascript:void(0)" title="查看会计分录" onclick="getVoucherEntryModal('PAY','${param.payHeadCode}')"><i class="fa fa-list-alt text-success"></i>&nbsp;&nbsp;<strong>查看会计分录</strong></a></li>
                        			<li><a href="javascript:void(0)" title="重新生成分录" onclick="autoCreateVoucherEntry('${param.payHeadCode}')"><i class="fa fa-calculator text-warning"></i>&nbsp;&nbsp;<strong>重新生成分录</strong></a></li>
                        		</c:when>
                        		<c:otherwise>
							...
							<div class="col-sm-12 col-sm-offset-2 text-right">
								...
								<c:if test="${param.payHeadCode!=null&&param.payHeadCode!=''}">
									...
									<c:if test="${requestScope.payHead.approveStatus=='SUBMIT' }">
										...
										<button class="btn btn-danger btn-lg btn-redragon-approve" type="button" onclick="window.location.href='web/apPayHead/updateApproveStatus?code=${requestScope.payHead.payHeadCode}&approveStatus=REJECT'">&nbsp;&nbsp;驳回&nbsp;&nbsp;<i class="fa fa-times-circle"></i></button>&nbsp;
									</c:if>
...
<script>
	...
	$(document).ready(function() {
		...
		//切换供应商验证
		$("#vendorCode").on("select2:open",function(){
	...
	function autoCreateVoucherEntry(headCode){
		$.ajax({
			type: "post",
			url: "web/apPayHead/autoCreateVoucherEntry",
	...
	function approveData(){
		redragonJS.confirm("确认审批通过？", function(){
			window.location.href='web/apPayHead/updateApproveStatus?code=${requestScope.payHead.payHeadCode}&approveStatus=APPROVE';
		});
	}
	...
	function alterData(){
		redragonJS.confirm("确认变更数据？数据变更后将产生变更历史信息！", function(){
			window.location.href='web/apPayHead/updateApproveStatus?code=${requestScope.payHead.payHeadCode}&approveStatus=UNSUBMIT';
		});
	}
</script>