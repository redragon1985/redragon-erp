...
<div class="wrapper wrapper-content animated fadeInRight">
	...
	<%-- 导入查询框 --%>
	<jsp:include page="search/mdCustomerSearch.jsp"></jsp:include>
			...
			<div class="ibox ">
		        <div class="ibox-title">
		            ...
		            <div class="ibox-tools">
		                <button id="addButton" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus"></i>&nbsp;&nbsp;<span class="bold">新增客户</span></button>
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover table-bordered border-top">
							...
							<tbody>
							    ...
							    <c:forEach items="${requestScope.mdCustomerList}" var="data" varStatus="status">
								<tr>
									<%-- 
									...
									<td>
										<c:choose>
											<c:when test="${data.customerType=='company'}">
												公司
											</c:when>
											<c:when test="${data.customerType=='person'}">
												个人
											</c:when>
										</c:choose>
									</td>
									<td>${data.customerAddress}</td>
									<td>${data.customerTelephone}</td>
									<td>${requestScope.cityMap[data.customerCity]}</td>
									...
									<td>
										<div class="btn-group">
											<button class="btn-white btn btn-xs btn-notcontrol" onclick="editSysUser('${data.customerCode}')"><i class="fa fa-edit"></i>&nbsp;编辑</button>&nbsp;
											<c:if test="${data.approveStatus!='APPROVE'&&data.approveStatus!='SUBMIT'}">
												<button class="btn-white btn btn-xs" onclick="deleteSysUser(${data.customerId},'${data.customerCode}')"><i class="fa fa-trash"></i>&nbsp;删除</button>
											</c:if>
										</div>
									</td>
								</tr>
...
<script>
	$(document).ready(function() {
		...
		*/
		$("#addButton").click(function(){
		    window.location.href="web/mdCustomer/getMdCustomer";
		});
	...
	function editSysUser(code){
		window.location.href="web/mdCustomer/getMdCustomer?customerCode="+code;
	}
	
	function deleteSysUser(id, code) {
		redragonJS.confirm("确认删除数据？", function(){
			window.location.href="web/mdCustomer/deleteMdCustomer?customerId="+id+"&customerCode="+code;
		});
	}
</script>       