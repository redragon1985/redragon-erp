...
<div class="wrapper wrapper-content animated fadeInRight">
	...
	<%-- 导入查询框 --%>
	<jsp:include page="search/arReceiptSearch.jsp"></jsp:include>
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover table-bordered border-top">
							...
							<tbody>
							    ...
							    <c:forEach items="${requestScope.arReceiptHeadList}" var="data" varStatus="status">
								<tr>
									<%-- 
									...
									<td>
										<div class="btn-group">
											<button class="btn-white btn btn-xs btn-notcontrol" onclick="editData('${data.receiptHeadCode}')"><i class="fa fa-edit"></i>&nbsp;编辑</button>&nbsp;
											<c:if test="${data.approveStatus!='APPROVE'&&data.approveStatus!='SUBMIT'}">
												<button class="btn-white btn btn-xs" onclick="deleteData(${data.receiptHeadId},'${data.receiptHeadCode}','${data.status}')"><i class="fa fa-trash"></i>&nbsp;删除</button>
											</c:if>
										</div>
									</td>
								</tr>
...
<script>
	$(document).ready(function() {
		...
		*/
		$("#addButton").click(function(){
		    window.location.href="web/arReceiptHead/getArReceiptHead";
		});
	...
	function editData(code){
		window.location.href="web/arReceiptHead/getArReceiptHead?receiptHeadCode="+code;
	}
	
	function deleteData(id, code, status) {
		redragonJS.confirm("确认删除数据？", function(){
			window.location.href="web/arReceiptHead/deleteArReceiptHead?receiptHeadId="+id+"&receiptHeadCode="+code+"&status="+status;
		});
	}
</script>       