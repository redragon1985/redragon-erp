			...
			<div class="modal-body" style="padding-bottom: 20px;">
				...
				<form id="lineForm">
					...
					<div class="form-group row">
						...
						<div class="col-sm-4 input-group">
							<input id="madeInvoiceQuantity" type="text" class="form-control" value="${requestScope.receiptLine.madeInvoiceQuantity}" readonly="readonly">
							<span class="input-group-addon">(${requestScope.receiptLine.unit})</span>
						</div>
					</div>
					<div class="hr-line-dashed"></div>
					...
					<input type="hidden" id="invoiceLineId" name="invoiceLineId" value="${requestScope.receiptLine.invoiceLineId}"> 
					<input type="hidden" id="invoiceLineCode" name="invoiceLineCode" value="${requestScope.receiptLine.invoiceLineCode}"> 
					<input type="hidden" id="invoiceSourceLineCode" name="invoiceSourceLineCode" value="${requestScope.receiptLine.invoiceSourceLineCode}"> 
...
<script>
	$(document).ready(function() {
		...
		$("#lineForm").validate({
			...
			submitHandler: function(form) {
				...
				if(quantity>redragonJS.numberSub(outputQuantity, madeInvoiceQuantity)){
					submitFlag = "N";
					redragonJS.alert("发票行数量不能大于出库数量/销售数量("+outputQuantity+")-开票数量("+madeInvoiceQuantity+")");
				}
	...
	function editLine(){
		...
		$.ajax({
			type: "post",
			url: "web/arInvoiceLine/editArInvoiceLine",
