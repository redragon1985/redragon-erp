...
<div id="lineTab" class="tab-pane">
	<div class="panel-body" style="padding-bottom: 0px; border-bottom: 0px;">
		...
		<div class="col-lg-12 text-right" style="padding-right: 0px;">
			<button id="addButton" class="btn btn-info btn-sm" type="button" ><i class="fa fa-plus"></i>&nbsp;&nbsp;<span class="bold">新增收款行</span></button>
			<%-- 
		...
		<div class="table-responsive">
			<table class="table table-stripped table-hover table-bordered border-top">
				...
				<tbody>
					...
					<tr>
						<td>${status.count}</td>
						<td>${data.invoiceSourceLineCode}</td>
						<td>${data.materialCode}</td>
						<td>${data.materialName}</td>
						<td>${data.standard}</td>
						<td>${data.price}</td>
						<td>${data.outputQuantity}</td>
						<td>${data.unit}</td>
						...
						<td>
							<div class="btn-group">
								<button class="btn-white btn btn-xs" onclick="editData(${data.invoiceLineId})"><i class="fa fa-edit"></i>&nbsp;编辑</button>&nbsp;
								<button class="btn-white btn btn-xs" onclick="deleteData(${data.invoiceLineId})"><i class="fa fa-trash"></i>&nbsp;删除</button>
							</div>
						</td>
					</tr>
					</c:forEach>
...
<script>
	...
	function getLineModal(id, soLineCode, materialCode, materialName, price, outputQuantity, unit, lineAmount){
		$.ajax({
			type: "post",
			url: "web/arInvoiceLine/getArInvoiceLine",
			data: {"invoiceLineId": id, "invoiceHeadCode": "${param.invoiceHeadCode}", "invoiceSourceType": $("#invoiceSourceType").val(), "invoiceSourceLineCode": soLineCode, "materialCode": materialCode,
				   "materialName": materialName, "price": price, "outputQuantity": outputQuantity, "unit": unit, "soLineAmount": lineAmount},
			async: false,
			dataType: "html",
			cache: false,
	...
	function deleteLine(id){
		$.ajax({
			type: "post",
			url: "web/arInvoiceLine/deleteArInvoiceLine",
			data: {"invoiceLineId": id, "invoiceHeadCode": "${param.invoiceHeadCode}"},
			async: false,
			dataType: "html",
			cache: false,
			success: function(data){
				...
				if(json.result=="success"){
					redragonJS.close();
					getLineTab("${param.invoiceHeadCode}");
				}else{
	...
	function getSelectSOLineModal(){
		...
		$.ajax({
			type: "post",
			url: "web/arInvoiceLine/getSelectSOLineModal",
			data: {"soHeadCode": $("#invoiceSourceHeadCode").val(), "invoiceHeadCode": "${param.invoiceHeadCode}"},
			async: false,
			dataType: "html",
			cache: false,
			success: function(data){
