						...
						<tbody>
							...
							<c:forEach items="${requestScope.poHeadList}" var="data" varStatus="status">
								<tr>
									...
									<td class="code">${data.poHeadCode}</td>
									<td class="name">${data.poName}</td>
									<td>${requestScope.poTypeMap[data.poType]}</td>
									<td class="companyName">${requestScope.vendorMap[data.vendorCode]}</td>
									<td class="companyCode" style="display: none;">${data.vendorCode}</td>
									<td><fmt:formatDate value="${data.signDate}" pattern="yyyy-MM-dd" /></td>
									<td>${data.staffName}</td>
									<td>${data.departmentName}</td>
									<td><span class="label">${requestScope.poStatusMap[data.status]}</span></td>
									<td class="companyContact" style="display: none;">${data.vendorContactDesc}</td>
								</tr>
							</c:forEach>
		...
		$("#selectButton").click(function(){
			if($("#lineTab table tbody tr").length>0){
				...
				if(selectFlag=="N"){
					...
					$("#vendorContactDesc").val(companyContact);
					$("#inputSourceHeadCode").val(code);
					$("#inputSourceHeadName").val(name);
