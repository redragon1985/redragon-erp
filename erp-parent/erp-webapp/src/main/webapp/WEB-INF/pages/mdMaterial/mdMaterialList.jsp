...
<div class="wrapper wrapper-content animated fadeInRight">
	...
	<%-- 导入查询框 --%>
	<jsp:include page="search/mdMaterialSearch.jsp"></jsp:include>
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover table-bordered border-top">
							...
							<tbody>
							    ...
							    <c:forEach items="${requestScope.mdMaterialList}" var="data" varStatus="status">
								<tr>
									<td>${status.count}</td>
									<td>${requestScope.materialTypeMap[data.materialType]}</td>
									<td>${data.materialCode}</td>
									<td>${data.materialName}</td>
									<td>${data.categoryDesc}</td>
									<td>${requestScope.materialUnitMap[data.materialUnit]}</td>
									<td>${data.standard}</td>
									<td>${requestScope.materialUnitMap[data.standardUnit]}</td>
...
<script>
	$(document).ready(function() {
		$("#addButton").click(function(){
		    window.location.href="web/mdMaterial/getMdMaterial";
		});
	...
	function editData(id){
		window.location.href="web/mdMaterial/getMdMaterial?materialId="+id;
	}
	
	function deleteData(id) {
		redragonJS.confirm("确认删除数据？", function(){
			window.location.href="web/mdMaterial/deleteMdMaterial?materialId="+id;
		});
	}
</script>       