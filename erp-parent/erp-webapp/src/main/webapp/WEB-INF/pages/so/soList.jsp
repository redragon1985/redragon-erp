			...
			<div class="ibox ">
		        <div class="ibox-title">
		            ...
		            <div class="ibox-tools">
		                <button id="addButton" class="btn btn-success btn-sm" type="button"><i class="fa fa-plus"></i>&nbsp;&nbsp;<span class="bold">新增销售订单</span></button>
								...
								<tr>
									...
									<c:choose>
									   <c:when test="${data.status=='NEW'}">
									       <span class="label label-primary">${requestScope.soStatusMap[data.status]}</span>
									   </c:when>
									   <c:when test="${data.status=='CONFIRM'}">
									       <span class="label label-success">${requestScope.soStatusMap[data.status]}</span>
									   </c:when>
									   <c:otherwise>
									       <span class="label label-danger">${requestScope.soStatusMap[data.status]}</span>
									   </c:otherwise>
									</c:choose>
									</td>
									<td>
									...
									<td>
										<div class="btn-group">
											<button class="btn-white btn btn-xs btn-notcontrol" onclick="editData('${data.soHeadCode}')"><i class="fa fa-edit"></i>&nbsp;编辑</button>&nbsp;
											<c:if test="${data.approveStatus!='APPROVE'&&data.approveStatus!='SUBMIT'}">
												<button class="btn-white btn btn-xs" onclick="deleteData(${data.soHeadId},'${data.soHeadCode}','${data.status}')"><i class="fa fa-trash"></i>&nbsp;删除</button>
											</c:if>
										</div>
									</td>
								</tr>
...
<script>
	$(document).ready(function() {
		...
		*/
		$("#addButton").click(function(){
		    window.location.href="web/soHead/getSoHead";
		});
	...
	function editData(code){
		window.location.href="web/soHead/getSoHead?soHeadCode="+code;
	}
	
	function deleteData(id, code, status) {
		redragonJS.confirm("确认删除数据？", function(){
			window.location.href="web/soHead/deleteSoHead?soHeadId="+id+"&soHeadCode="+code+"&status="+status;
		});
	}
</script>       