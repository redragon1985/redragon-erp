...
public class PersonDirectoryPrincipalResolver implements PrincipalResolver {
    ...
    public final Principal resolve(final Credential credential) {
    	RememberMeUsernamePasswordCredential rupcCredential=(RememberMeUsernamePasswordCredential)credential;
        logger.debug("Attempting to resolve a principal...");
        //String orgcode=rupcCredential.getOrgcode();//组织机构
        String principalId = extractPrincipalId(credential);//用户名
        String param="";//参数
        if (principalId == null) {
...
//        if (orgcode == null) {
//            logger.debug("Got null for extracted principal orgcode; returning null.");
//            return null;
//        }

        //logger.debug("Creating SimplePrincipal for [{}]", principalId,orgcode);
        logger.debug("Creating SimplePrincipal for [{}]", principalId);
        //param=principalId+","+orgcode;
        param=principalId;
        final IPersonAttributes personAttributes = this.attributeRepository.getPerson(param);
